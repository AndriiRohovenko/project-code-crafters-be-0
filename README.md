...
> 💡 Ця команда очищає всі таблиці, скидає AUTO_INCREMENT послідовності та запускає всі seeders заново. Використовуйте її замість `db:seed:undo:all && db:seed:all` для коректного перезаповнення бази даних.
...
- `npm run db:reseed` - ⭐ Скинути та перезаповнити БД (рекомендовано)
- `npm run db:reset` - Очистити всі таблиці
